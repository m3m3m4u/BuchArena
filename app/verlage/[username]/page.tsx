...
export default function VerlageProfilePage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      try {
        ...
        setMessage("Verlagsprofil konnte nicht geladen werden.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const visibleName = useMemo(() =>
    verlageProfile.name.visibility === "public" && verlageProfile.name.value
      ? verlageProfile.name.value
      : username,
  [verlageProfile.name, username]);

  const kapazitaeten = useMemo(() =>
    Array.isArray(verlageProfile.kapazitaeten) ? verlageProfile.kapazitaeten : [],
  [verlageProfile.kapazitaeten]);

  const socialLinks = useMemo(() => {
    return [
      { label: "Website", field: verlageProfile.socialWebsite },
      { label: "Instagram", field: verlageProfile.socialInstagram },
      { label: "Facebook", field: verlageProfile.socialFacebook },
      { label: "LinkedIn", field: verlageProfile.socialLinkedin },
      { label: "TikTok", field: verlageProfile.socialTiktok },
      { label: "YouTube", field: verlageProfile.socialYoutube },
      { label: "Pinterest", field: verlageProfile.socialPinterest },
      { label: "Reddit", field: verlageProfile.socialReddit },
      { label: "Linktree", field: verlageProfile.socialLinktree },
      { label: "Newsletter", field: verlageProfile.socialNewsletter },
      { label: "WhatsApp-Kanal", field: verlageProfile.socialWhatsapp },
      { label: "Mailadresse", field: verlageProfile.socialEmail },
