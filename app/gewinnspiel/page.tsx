  ...
  return (
    <main className="site-shell py-8">
      ...
      <div className="flex gap-1 mb-6 border-b" style={{ borderColor: "var(--color-arena-border)" }}>
        ...
        >
          Archiv {archiv.length > 0 && <span className="ml-1 text-xs bg-gray-200 text-gray-700 px-1.5 py-0.5 rounded-full">{archiv.length}</span>}
        </button>
      </div>
                ...
                <div className="p-4 flex flex-col flex-1 min-w-0">
                  ...
                  <div className="mt-auto text-xs opacity-60 flex flex-wrap gap-x-4 gap-y-1">
                    ...
                    {g.status !== "anmeldung" && g.gewinnerName && (
                      <span className="text-green-700 font-medium">Gewinner: {g.gewinnerName}</span>
                    )}
                  </div>
                </div>
              </Link>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-6">
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium border disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100 transition-colors"
                style={{ borderColor: "var(--color-arena-border)" }}
              >
                Zuruck
              </button>
